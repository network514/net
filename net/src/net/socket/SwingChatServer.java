...
public class SwingChatServer extends JFrame implements ActionListener{
	...
	/*ArrayList를 사용하지 않고 Vector를 사용하는 것은
	 * 동기화 지원 때문이다. 소케스로그램은 동기화 지원이 필요하다.*/
	...
	public SwingChatServer() {
		...
		setVisible(true);
		//이벤트 처리 ---------------------------
		super.setDefaultCloseOperation(EXIT_ON_CLOSE); // 윈도우 닫기 이벤트
		btnExit.addActionListener(this);
		inwon = new Vector(); // 백터에서 제네릭을 제거하면 모든 타입 수용
		serverStart();
	...
	private void serverStart() {
		...
		try{
			...
			txtList.setFont(new Font("굴림체", Font.BOLD, 16)); // 12는 크기
			while (true){
				Socket socket = ss.accept();
				String str = socket.getInetAddress().getHostAddress();
				txtList.append(str);
				// 병행처리를 하기위한 클라이언트 객체생성 (사용자 정의 클래스)
			}
		}catch(Exception ex){
			ex.printStackTrace();
		}
...
class ChatHandle extends Thread{
	...
	public void run() {
		...
		try{
			...
			server.setMsg("["+nickName+"] 님이 입장하셨습니다."); // 자동 생성된 메소드로 가기 위한 단축키 CTRL + 마우스 클릭
